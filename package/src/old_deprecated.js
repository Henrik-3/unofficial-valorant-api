...
module.exports = {
    ...
    returndata: async function (url, type, data) {
        if (url == 'matchuuidvalidation')
            return {status: null, data: null, ratelimits: null, error: {message: ' Match UUID Validation failed, make sure you entered a valid Match UUID'}};
        if (url == 'puuidvalidation')
            return {status: null, data: null, ratelimits: null, error: {message: 'PUUID Validation failed, make sure you entered a valid PUUID'}};
        const response =
        ...
        if (response.response)
            return {
                ...
                ratelimits: {
                    used: response.response.headers['x-ratelimit-limit'],
                    remaining: response.response.headers['x-ratelimit-remaining'],
                    reset: response.response.headers['x-ratelimit-reset'],
                },
                error: null,
            };
        return this.links.includes(type)
            ? {
                  status: response.status,
                  data: response.data,
                  ratelimits: {
                      used: response.headers['x-ratelimit-limit'],
     ...
     * @property {String} tag Tag of the player
     */
    ...
    getMMR: async function (version, region, name, tag, filter) {
        return this.returndata(
            `https://api.henrikdev.xyz/valorant/${version}/mmr/${region}/${encodeURI(name)}/${encodeURI(tag)}${filter != undefined ? `?filter=${filter}` : ''}`,
            'mmr'
        );
    },
    /**
    ...
    getMMRByPUUID: async function (version, region, puuid, filter) {
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(puuid))
            return this.returndata(
                `https://api.henrikdev.xyz/valorant/${version}/by-puuid/mmr/${region}/${puuid}${filter != undefined ? `?filter=${filter}` : ''}`,
                'mmr'
            );
    ...
    getMMRHistoryByPUUID: async function (region, puuid) {
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(puuid))
            return this.returndata(`https://api.henrikdev.xyz/valorant/v1/by-puuid/mmr-history/${region}/${puuid}`, 'mmr');
        return this.returndata('puuidvalidation');
    },
    /**
     * Get Match by ID
    ...
    getMatch: async function (uuid) {
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(uuid))
            return this.returndata(`https://api.henrikdev.xyz/valorant/v2/match/${uuid}`, 'matches');
    ...
    getMatches: async function (region, name, tag, size, filter) {
        return this.returndata(
            `https://api.henrikdev.xyz/valorant/v3/matches/${region}/${encodeURI(name)}/${encodeURI(tag)}${size ? `?size=${size}` : ''}${
    ...
    getMatchesByPUUID: async function (region, puuid, size, filter) {
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(puuid))
            return this.returndata(
                `https://api.henrikdev.xyz/valorant/v3/by-puuid/matches/${region}/${puuid}${
                    size != undefined && filter == undefined
                        ? `?size=${size}`
                        : size == undefined && filter != undefined
                        ? `?filter=${filter}`
                        : size != undefined && filter != undefined
        ...
        return this.returndata(
            `https://api.henrikdev.xyz/valorant/v1/leaderboard/${region}${
                name != undefined && tag == undefined
                    ...
                    ? ''
                    : name != undefined && tag != undefined
                    ? `?name=${encodeURI(name)}&tag=${encodeURI(tag)}`
                    : ''
            }`,
            'leaderboard'
        );
