...
export type LeaderboardResponse<T> =
	T extends "v1" ? V1LeaderboardResponse :
    T extends "v2" ? V2LeaderboardResponse :
    V2LeaderboardResponse;

export type MMRResponse<T> =
    T extends "v1" ? V1MMRResponse :
    T extends "v2" ? V2MMRResponse :
    V2MMRResponse;

export type StoreFeaturedResponse<T> =
    T extends "v1" ? V1StoreFeaturedResponse :
    T extends "v2" ? V2StoreFeaturedResponse :
    V2StoreFeaturedResponse;

export type RawResponse<T> =
	T extends "matchdetails" ? RawMatchDetailsResponse :
    T extends "matchhistory" ? RawMatchHistoryResponse :
	T extends "mmr" ? RawMMRResponse :
    T extends "competitiveupdates" ? RawCompetitiveUpdatesResponse :
    never;
