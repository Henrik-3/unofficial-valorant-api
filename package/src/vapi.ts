...
export default class {
    ...
    async getCrosshair({ code, size }: { code: string, size?: number }): Promise<APIResponse<V1CrosshairResponse>> {
        ...
        return this.fetch<V1CrosshairResponse>("v1/crosshair/generate", { id: code, size }, {
	 ...
	 */
    async getAnnouncements({ countryCode }: { countryCode: "en-us" | "en-gb" | "de-de" | "es-es" | "es-mx" | "fr-fr" | "it-it" | "ja-jp" | "ko-kr" | "pt-br" | "ru-ru" | "tr-tr" | "vi-vn" }): Promise<APIResponse<V1WebsiteResponse>> {
	 ...
	 */
    async getWebsite({ countryCode }: { countryCode: "en-us" | "en-gb" | "de-de" | "es-es" | "es-mx" | "fr-fr" | "it-it" | "ja-jp" | "ko-kr" | "pt-br" | "ru-ru" | "tr-tr" | "vi-vn" }): Promise<APIResponse<V1WebsiteResponse>> {
        this.validateArgs({ countryCode });
        return this.fetch<V1WebsiteResponse>(`v1/website/${countryCode}`);
    }
	 ...
	 */
    async getRawData<T extends "matchdetails" | "matchhistory" | "mmr" | "competitiveupdates">({ type, value, region, queries }: { type: T, value: string, region: Region, queries: any }): Promise<APIResponse<RawResponse<T>>> {
    ...
    */
	 ...
	 */
    async getLeaderboard<T extends "v1" | "v2">({ version, region, start, end, riotID, puuid, season }: { version?: T, region: Region, start?: number, end?: number, riotID?: { name: string, tag: string} , puuid?: string, season?: Season }): Promise<APIResponse<LeaderboardResponse<T>>> {
        if (version === "v1") {
	 ...
	 */
    async getMMR<T extends "v1" | "v2">({ name, tag, region, seasonFilter, version, filter }: { name: string, tag: string, region: Region, seasonFilter?: Season, version?: T, filter?: Season }): Promise<APIResponse<MMRResponse<T>>> {
	 ...
	 * @throws TypeError - If both filter and seasonFilter are set
	 */
    async getMMRByPUUID<T extends "v1" | "v2">({ puuid, region, seasonFilter, version, filter }: { puuid: string, region: Region, seasonFilter?: Season, version?: T, filter?: Season }): Promise<APIResponse<MMRResponse<T>>> {
        if (version === "v1") console.warn("v1 is outdated, please migrate to v2!");
        if (filter) console.warn("filter is outdated! Please use seasonFilter instead");
        if (seasonFilter && filter) throw new TypeError("seasonFilter and filter cannot be both set!");
	...
	*/
    async getMatches({ name, tag, region, gamemodeFilter, mapFilter, size }: { name: string, tag: string, region: Region, gamemodeFilter?: Mode, mapFilter?: ValorantMap, size?: number }): Promise<APIResponse<V3MatchesResponse>> {
        this.validateArgs({ name, tag, region });
	 ...
	 * @throws TypeError - Only if the size parameter is set and not between 1-10
	*/
    async getMatchesByPUUID({ puuid, region, gamemodeFilter, mapFilter, size }: { puuid: string, region: Region, gamemodeFilter?: Mode, mapFilter?: ValorantMap, size?: number }): Promise<APIResponse<V3MatchesResponse>> {
